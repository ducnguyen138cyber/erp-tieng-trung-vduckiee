...
const hsk3Exercises = JSON.parse(fs.readFileSync(path.join(root, "data/hsk/hsk3/exercises.json"), "utf8")).records;
...
const sizes = [
  { name: "desktop-1440", width: 1440, height: 900, lesson: "hsk3-lesson-01" },
  { name: "desktop-1024", width: 1024, height: 768, lesson: "hsk3-lesson-18" },
...
async function assertHsk3(page, state) {
  const expected = { units: 12, lessons: 36, grammar: 42, characters: 100, exercises: 252, assessments: 15, vocabulary: 500 };
  if (!state || state.readOnly !== true || state.progressWritesEnabled !== false || state.selectedLevel !== 3) throw new Error("HSK3 runtime is not selected/read-only.");
  for (const [key, value] of Object.entries(expected)) if (state.counts[key] !== value) throw new Error(`HSK3 count ${key}: ${state.counts[key]} != ${value}`);
  await page.locator(".hsk-intro strong").getByText("HSK 3 Professional · C4 learner-facing", { exact: true }).waitFor();
  const hsk3Button = page.locator('[data-pro-level="3"]');
  if (await hsk3Button.count() !== 1 || await hsk3Button.isDisabled()) throw new Error("HSK3 selector is still locked/coming soon.");
  if (await page.locator(".hsk-pro-unit").count() !== 12) throw new Error("HSK3 unit list is incomplete.");
  if (await page.locator("[data-pro-lesson]").count() !== 36) throw new Error("HSK3 lesson list is incomplete.");
  if (await page.locator("[data-pro-assessment]").count() !== 15) throw new Error("HSK3 assessment list is incomplete.");
  for (const heading of ["Từ vựng mới trong ngữ cảnh", "Chữ Hán trọng tâm", "Ngữ pháp để diễn đạt theo đoạn", "Hội thoại có mục tiêu", "Đọc hiểu và giải thích", "Nghe, chép chính tả và shadowing", "Phát âm cho người Việt", "Ghi chú văn hoá và ứng xử", "Nói, viết và dùng thật", "Ôn cách quãng", "Bài tập của lesson"]) {
...
(async () => {
  ...
  const browser = await chromium.launch({ headless: true, executablePath, args: executablePath ? ["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"] : [] });
  try {
    for (const spec of sizes) {
      ...
      await page.goto(`${base}?area=hsk&hskLevel=3&hskLesson=${spec.lesson}`, { waitUntil: "domcontentloaded", timeout: 30000 });
      if (cjkFontData) {
      ...
      const state = await waitReady(page, 3);
      await assertHsk3(page, state);
      await page.locator("#hskLesson").getByText(new RegExp(`BÀI\\s+${Number(spec.lesson.slice(-2))}\\s+\\/\\s+36`, "i")).first().waitFor();
      ...
      await page.screenshot({ path: `/tmp/hsk3-c4-${spec.name}.png`, fullPage: true });

      if (spec.name === "desktop-1440") {
        await page.locator('[data-pro-lesson="hsk3-lesson-02"]').click();
        await page.locator("#hskLesson").getByText("Nói rõ một sự việc", { exact: true }).waitFor();
        ...
        await page.locator("#hskLesson").getByText("Từ một câu đến một đoạn", { exact: true }).waitFor();
        const first = hsk3Exercises.find((exercise) => exercise.id === "hsk3-lesson-01-exercise-1");
        ...
        await page.locator('[data-pro-assessment="hsk3-assessment-unit-01"]').click();
        await page.locator("#hskLesson").getByText(/Checkpoint Unit 1/).waitFor();
        await page.locator('[data-pro-assessment="hsk3-assessment-midpoint"]').click();
        await page.locator("#hskLesson").getByText(/Midpoint/).waitFor();
        await page.locator('[data-pro-assessment="hsk3-assessment-final"]').click();
        await page.locator("#hskLesson").getByText(/Final Assessment/).waitFor();
        await page.locator('[data-pro-assessment="hsk3-assessment-mastery"]').click();
        await page.locator("#hskLesson").getByText(/Mastery Review/).waitFor();
        await page.locator('[data-pro-level="2"]').click();
        const hsk2 = await waitReady(page, 2);
        if (hsk2.counts.lessons !== 28 || hsk2.counts.vocabulary !== 200) throw new Error("HSK2 regressed after HSK3 integration.");
        await page.locator('[data-pro-level="1"]').click();
        const hsk1 = await waitReady(page, 1);
        if (hsk1.counts.lessons !== 24 || hsk1.counts.vocabulary !== 300) throw new Error("HSK1 regressed after HSK3 integration.");
        await page.locator('[data-pro-level="3"]').click();
    ...
    fs.writeFileSync("/tmp/hsk3-c4-browser-smoke.json", `${JSON.stringify(result, null, 2)}\n`);
    console.log(JSON.stringify(result, null, 2));
  } finally { await browser.close(); }
})().catch((error) => { result.error = error && error.stack || String(error); try { fs.writeFileSync("/tmp/hsk3-c4-browser-smoke.json", `${JSON.stringify(result, null, 2)}\n`); } catch (_) {} console.error(error); process.exit(1); });

(function (root, document) {
  ...
  function authorize(session, openAfterAuthorize) {
    var request = ++token;
  ...
  function onKey(event) {
    ...
    if (event.ctrlKey && event.shiftKey && String(event.key).toLowerCase() === "d") { event.preventDefault(); if (authorized) ns.ui.toggle(); else authorize(currentSession(), true); }
  }
  ...
  root.VDuckieDeveloperControlCenter=Object.freeze({version:"108.1",open:function(){if(authorized)ns.ui.open();else authorize(currentSession(), true);},close:function(){if(ns&&ns.ui)ns.ui.close();},destroy:destroy,isAuthorized:function(){return authorized;}});

(function(root){
  ...
  function formatCount(value){
    try{return Number(value).toLocaleString("vi-VN")+"+";}
  ...
  function apply(){
    ...
    setTile(".home-resource-tile.resource-erp",formatCount(dictionaryCount()),"Từ vựng");
    setTile(".home-resource-tile.resource-roast",HSK_QUIZ_QUESTION_COUNT+"+","Câu kiểm tra");

    if(!document.getElementById("home-copy-v79-style")){
      var style=document.createElement("style");
      style.id="home-copy-v79-style";
      style.textContent=[
  ...
  function delayedApply(){root.setTimeout(apply,0);}
  ...
  function install(){
    ...
    var timer=root.setInterval(function(){
      attempts++;
      apply();
      var dictionaryReady=root.VDuckieDictionary&&Array.isArray(root.VDuckieDictionary.records)&&root.VDuckieDictionary.records.length>=10000;
      if(attempts>=180||dictionaryReady)root.clearInterval(timer);
    },350);
    root.addEventListener("vduckie:erp-v77-ready",delayedApply);
    root.addEventListener("vduckie:hsk-dictionary-ready",delayedApply);
  }
